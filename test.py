        ...
        for row in reader:
            ...
            for page in pages:
                try:
                    ...
                    continue
            text_positions.append(row_positions)
...
def combine_data(text_positions_rows, coordinates):
    ...
    for text_positions in text_positions_rows:
        ...
        while text_index < len(text_positions):
            temp_list = []
            coord_index = 0  # Reset for each text_position set
...
def add_text_to_pdf(input_pdf_path, output_base_path, combined_text_data):
    for row_index, text_positions in enumerate(combined_text_data):
        writer = PdfWriter()
        reader = PdfReader(input_pdf_path)  # Open reader INSIDE the loop
        with pdfplumber.open(input_pdf_path) as pdf: # Open plumber INSIDE the loop
            grouped_positions = {}
            for item in text_positions:
            ...
            for page_number in range(len(reader.pages)):
                page = reader.pages[page_number] # Create a fresh page object
